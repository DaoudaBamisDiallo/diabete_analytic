...
def deployement():
        ...
        def inference(Glucose,BMI,Age,DiabetesPedigreeFunction,BloodPressure,Pregnancies):
            ...
            data = pd.DataFrame([df], columns=["Glucose","BMI","Age","DiabetesPedigreeFunction","BloodPressure","Pregnancies"])
            scal_df = scaler.fit_transform(data)
            st.write(data)
            st.write(scal_df.reshape(-1,1))
            # diabetique = model_diabete.predict(scal_df.reshape(1,-1))
            # return diabetique
